...
class Div2(Pretty):
  ...
  def __init__(i,lst, x, y, yis="Num"):
    i.yis    = yis
    list1 = lst.cols[x].numList
    if yis == "Num":
      list2 = lst.cols[y].numList
    else:
      list2 = lst.cols[y].symList
    i.data   = [list(a) for a in zip(list1, list2)]
    i.x_lst, i.y_lst = i.makeList(i.data, yis)
    i.b4     = i.y_lst
    i._lst   = sorted(i.data,key=lambda x: (x[0]))
  ...
  def __divide(i, lo, hi, xr, yr, rank):
    ...
    if cut:
      ...
      # print("best",i.best)
      # print("yr var", y_old.variety())
      if y_old.variety() < i.best:
        i.best = y_old.variety()
        i.cut = hi
        i.l = lo
      # if i.best > yr.variety():
      #   i.best = yr.variety()
      #   i.cut = lo
